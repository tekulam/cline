...
Jupyter notebooks are JSON files with a specific structure. When editing .ipynb files, you MUST match the exact JSON format including quotes, commas, and \\n characters as they appear in the raw file content.

### Critical Formatting Requirements

1. **Line Endings**: Each line in the "source" array (except the last) ends with "\\n"
2. **Array Format**: Each line of code is a separate JSON string in the "source" array
3. **Exact Matching**: In SEARCH blocks, match the JSON structure exactly as it appears in the file
4. **Quotes**: Use double quotes for JSON strings
5. **Commas**: Include commas between array elements

### Example: Adding a Line to a Notebook Cell

If the current cell looks like this:
\`\`\`json
{
  "cell_type": "code",
  "source": [
...
To add a new print statement, use replace_in_file:
\`\`\`
------- SEARCH
  "source": [
    "data = {\\n",
...
### Example: Modifying Code Within a Cell
...
❌ **Wrong**: Not matching the exact JSON structure
\`\`\`
------- SEARCH
data = {'x': 1}
=======
...
✅ **Correct**: Match the JSON array format
\`\`\`
------- SEARCH
  "source": [
    "data = {'x': 1}"
...
### When to Use write_to_file vs replace_in_file for Notebooks

- **Use replace_in_file** for:
  - Modifying individual cells
  - Adding/removing lines within a cell
  - Changing cell outputs or metadata
  
- **Use write_to_file** for:
  - Creating new notebooks
  - Completely restructuring the notebook
  - Adding/removing multiple cells at once

### Important Notes

- Always read the notebook file first to see the exact JSON structure
- The "source" array is the most commonly edited part
- Cell metadata, outputs, and execution_count can also be modified
- Preserve the overall notebook structure (cells array, metadata, nbformat, etc.)
{{/if}}`
