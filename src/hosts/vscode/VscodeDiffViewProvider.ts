...
export class VscodeDiffViewProvider extends DiffViewProvider {
	...
	override async replaceText(
		...
		// For notebook files with temp files, check if notebook diff view needs saving
		if (this.isNotebookFile() && this.tempModifiedUri) {
			try {
				// Find the notebook diff tab with our temp file
				const tabs = vscode.window.tabGroups.all.flatMap((tg) => tg.tabs)
				const notebookDiffTab = tabs.find(
					(tab) =>
						tab.input instanceof vscode.TabInputNotebookDiff &&
						tab.input.modified.fsPath === this.tempModifiedUri!.fsPath,
				)

				Logger.log(`Notebook diff tab found: ${!!notebookDiffTab}`)

				if (notebookDiffTab && notebookDiffTab.isDirty) {
					Logger.log("Notebook diff tab is dirty")

					// Check if this tab is currently active
					const activeTab = vscode.window.tabGroups.activeTabGroup.activeTab
					Logger.log(`Active tab matches notebook diff tab: ${activeTab === notebookDiffTab}`)

					if (activeTab === notebookDiffTab) {
						// The notebook diff view is active, save it
						Logger.log("Notebook diff view is active, executing save command...")
						await vscode.commands.executeCommand("workbench.action.files.save")

						// Give VS Code a moment to complete the save
						await new Promise((resolve) => setTimeout(resolve, 200))

						// The file watcher will trigger sync automatically, but we can also manually sync
						Logger.log("Syncing temp file to active diff editor...")
						await this.syncTempFileToActiveDiffEditor()
						Logger.log("Sync completed")
					} else {
						Logger.log("Notebook diff tab found but not active, cannot save automatically")
						// The notebook diff view is not active, we can't save it automatically
						// The user will need to save it manually or make it active
					}
				} else if (notebookDiffTab && !notebookDiffTab.isDirty) {
					Logger.log("Notebook diff tab found but not dirty, syncing anyway...")
					// Even if not dirty, sync to ensure consistency
					await this.syncTempFileToActiveDiffEditor()
				} else {
					Logger.log("No notebook diff tab found")
				}
			} catch (error) {
				Logger.error("Failed to save notebook diff view:", error)
	...
	protected override async resetDiffView(): Promise<void> {
		// Clean up temporary files and listeners
		await this.cleanupTempFiles()
